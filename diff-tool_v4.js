use(function () {
    ...
    while (searchtree.length > 0) {
        ...
        try {
            ...
            while (searchResult.hasNext()) {
                allDiffPath.push(searchResult.next().getPath());
            }
        } catch (err) {
    ...
    let pathCountMax = 100;
    if(allDiffPath.length > pathCountMax) {
        while(allDiffPath.length > 0){
            let item = {paths: []};
            while(item.paths.length < pathCountMax){
                item.paths.push(allDiffPath.pop());
            }
